...
{
public:
	textureLetters(int number = 0, float inputW = 0.08, float inputH = 0.15, float posX = -1.0, float posY = -1.0)
	{
		if (number > 0)
			NUM_BLOCK = number;
		else
		...
		for (int i = 0; i < 38; i++)
		{
			...
			if (letter != '?')
				filename << "letters/" << letter << ".bmp";
			else
				filename << "letters/.bmp";

			string texture_name = filename.str();
			letter_texture[i] = LoadTexture(texture_name.c_str());
		}
	}

	void CreateVao(string inputS)
	{
		contents = inputS;
	...
	void Display()
	{
		for (int i = 0; i < NUM_BLOCK; i++)
		{
			...
			for (int i = 0; i < contents.length(); i++)
			{
				...
				glBindTexture(GL_TEXTURE_2D, letter_texture[digit]);
